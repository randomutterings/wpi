class Listing < ActiveRecord::Base
  ...
  def self.search(params)
    ...
    if params[:max_price].blank?
      ...
      params[:max_price] = params[:max_price].to_i + 1
      params[:max_price] = params[:max_price].to_s
    end
    conditions = Hash.new(true)
    conditions["price"] = params[:min_price]...params[:max_price]
    ...
    conditions["county"] = params[:county] unless params[:county].blank?
    conditions["bedrooms"] = params[:bedrooms] unless params[:bedrooms].blank?
    conditions["bathrooms"] = params[:bathrooms] unless params[:bathrooms].blank?
    listings = self.all(:conditions => conditions)
  end
